...
if TrialJND.Gabor_position(trial) == 'R' % 1 for left and 2 for right
    TrialJND.contrast_right(trial) = (10.^(tr)) + TrialJND.Gabor_contrast(trial);
    TrialJND.contrast_left(trial) = TrialJND.Gabor_contrast(trial);
    TrialJND.contrast(trial,2) = (10.^(tr)) + TrialJND.Gabor_contrast(trial);
    TrialJND.contrast(trial,1) = TrialJND.Gabor_contrast(trial);
else %'L'
    TrialJND.contrast_left(trial) = (10.^(tr)) + TrialJND.Gabor_contrast(trial);
    TrialJND.contrast_right(trial) = TrialJND.Gabor_contrast(trial);
    TrialJND.contrast(trial,1) = (10.^(tr)) + TrialJND.Gabor_contrast(trial);
    TrialJND.contrast(trial,2) = TrialJND.Gabor_contrast(trial);
end

% store some date
TrialJND.log10_tr(trial) = tr;
TrialJND.contrast_tr(trial) = (10.^(tr)) + TrialJND.Gabor_contrast(trial);
TrialJND.contrast_B(trial) = TrialJND.Gabor_contrast(trial);
TrialJND.JND_contrast(trial) = (10.^tr); %TrialJND.contrast_tr(trial)-TrialJND.contrast_B(trial);
TrialJND.JND_log10contrast(trial) = tr; %TrialJND.log10_tr(trial)-log10(TrialJND.contrast_B(trial));
